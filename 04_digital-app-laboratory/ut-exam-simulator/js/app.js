...
const app = {
    ...
    async renderDashboard() {
        ...
        listContainer.innerHTML = '<div class="text-center p-4">Loading...</div>';
    ...
    async startExam(examId) {
        ...
        if (!session) {
            session = {
                id: crypto.randomUUID(),
                examId: examId,
                studentInfo: { name: 'Mahasiswa', nim: '052203399' },
                answers: {},
                startTime: Date.now(),
                timeLeft: 3600, // 60 mins default
                isComplete: false
            };
        ...
        document.getElementById('sim-nim').textContent = session.studentInfo.nim;
    ...
    startTimer() {
        ...
        const updateTimer = async () => {
            ...
            // Save every 30 seconds or so (or on every tick if efficient? indexDB is fast enough for 1 tick/sec usually but safer to throttle)
            if (s.timeLeft % 5 === 0) {
    ...
    async renderCurrentPage() {
        ...
        try {
            ...
            // Calc container width for "Fit Width"
            const containerWidth = document.getElementById('pdf-wrapper').clientWidth;
            const viewport = page.getViewport({ scale: this.state.scale }); // Base scale

            // Adjust to fit width logic? 
            // Simplified: Use fixed scale for now, but UI says "Fit Width".
            // Let's compute scale based on container width
